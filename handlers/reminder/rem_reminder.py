...
async def stop_remove_reminder(message: types.Message, state: FSMContext):
    ...
    if len(list_of_reminders) < position: return await message.answer(
        'Циферка больше, чем последняя циферка списка, введите ещё одну')
    ...
    await message.answer(
        'Данные удалены' if rem_reminder_db(text_in_reminder, date_in_reminder) else "Ошибка удаления, отмена команды")
    await state.finish()
